...
K_means<-function(X,gamma,delta, case){
  result <- ADMM(X, gamma, delta) ## gamma : 0.01, delta : 0.03 ##
  a<-result[[1]]
  M<-result[[2]]
  L<-result[[3]]
  S<-result[[4]]
  ...
  N = ncol(X)
  First_eigen = eigen(L)$vectors[,1]
  Second_eigen = eigen(L)$vectors[,2]
  Third_eigen = eigen(L)$vectors[,3]
  Label = seq(from=1,to=N,by=1)
  
  Indx = c();
  for(i in 1:K){
    Indx = c(Indx,rep(i,N/K))
  }
  
  KMeans = kmeans(eigen(L)$vectors[,1:K], K, iter.max = 1000, nstart = 100, 
                  algorithm = "Hartigan-Wong")
  plot(eigen(L)$vectors[,1:2], col=KMeans$cluster,pch=0, 
       main=TeX(sprintf("$\\textbf{Case  %d}$", case)),
       xlab="First Eigen vector", ylab="Second Eigen vector")
  # text(eigen(L)$vectors[,1],eigen(L)$vectors[,2],labels=Indx,cex= 1.5, pos=3)
  #scatter3D(eigen(L)$vectors[,1], eigen(L)$vectors[,2], eigen(L)$vectors[,3], 
  #          theta = 15, phi = 20)
}
