...
Res <- ADMM(Net2[[1]],gamma[1],delta[8]);
qr(Res[[3]])$rank

Eval_func(Net2,gamma[1],delta[8],1) # AIC
Eval_func(Net2,gamma[Case2[1,1]],delta[Case2[[2]][1,2]],1) # BIC
...
Eval_func(Net2,gamma[4],delta[3],1) # NHCV
...
######################### Scenario 2 #########################
######################### Case 4 #########################
gamma = seq(from=0.0038,to=0.0048,by=0.0001);
delta = seq(from=0.055,to=0.065,by=0.001);
...
for(i in 1:length(gamma)){
  for(j in 1:length(delta)){
    MisCl_rate4[i,j] <- CV(Net4[[1]][[1]],gamma[i],delta[j],5)    
    print(count)
    count <- count + 1;
  }
}
