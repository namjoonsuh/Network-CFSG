...
\begin{remark}
Throughout the Section, we add $*$ to the superscript of parameters to denote that they are ground truth parameters.
We use $\alpha^*$ as an intercept term in logistic regression model, and
$f^*_j$ as $j$th column of factor loading matrix $F^* \in \mathbb{R}^{K \times n}$, where $K$ refers to the number of topics embedded in network, and $n$ denotes the number of papers in the network.
We also introduce a notation, $n_{l}$, denoting the number of papers, which share $l$ common topics.
Matrix $D^* \in \mathbb{R}^{K \times K}$ is a diagonal matrix whose entries denote weight coefficient of factors, and
$S_{ij}^*$ denotes an ad-hoc citational relation between $i$th and $j$th paper.
We use the notation $|(\bullet)|$ to denote the cardinality of non-zero components of upper-triangular part of $(\bullet)$ matrix
(e.g., $|S^*|=|\{(i,j) : S_{ij}^{*}\neq 0, 1 \leq i<j \leq n\}|$).
We will use $X \sim \text{Unif}[a,b]$ to denote
the random variable $X$ is drawn from uniform distribution supported on the interval $[a,b]$.
\end{remark}

\subsection{Numerical experiments with synthetic data}
\label{sec:Syn-dat}
After an introduction on three synthetic scenarios that we want to explore, three model selection criteria and three evaluation metrics for the selected model will be suggested.
Experimental results from the synthetic networks and several interesting findings from those results will be elaborated subsequently.

\begin{figure}[htbp]
\includegraphics[width=1\textwidth]{Fig1.pdf}
\caption{Graphical illustrations of $9$ synthetic networks. Note the links that are drawn from $S^*_{ij} \neq 0$ are colored in green. }
\label{fig:figure1}
\end{figure}

\subsubsection{Synthetic Setting}
In this subsection, we consider following $3$ scenarios:
...
\begin{itemize}
    \item \textbf{First scenario.} When there is a network of $n$ papers with $K$ topics, there are $K$ clusters of papers, where each of which deals with $1$ specific topic out of $K$ topics.
        And each cluster is comprised of $\frac{n}{K}$ non-overlapping papers.
        Moreover, these $K$ clusters are connected with each other via $|S^*|$ random ad-hoc links.
        Specifically, we assume $|S^*|/\binom{K}{2}$ ad-hoc links connect $2$ different clusters of papers.
        For example, suppose that we have a network of $120$ papers with $5$ topics and with $30$ random ad-hoc links. Then, there are $5$ clusters, each of which has $24$ papers, and each of $10 (=\binom{5}{2})$ pairs of clusters is connected through $3 (=30/\binom{5}{2})$ random ad-hoc links.
...
With these settings in mind, we construct $3$ cases of networks for scenarios $1$, $2$, and $3$ respectively. Specifically;

\begin{enumerate}
    \item In the first scenario, we consider following three cases: $\{(n^{(i)},n_{1}^{(i)},K^{(i)},|S^*|^{(i)})\}_{i=1}^{3}$ $=\{(30,30,3,9), (80,80,4,18),$ $(120,120,5,30)\}$.
        ...
        Note that every paper in three networks, which are considered in this scenario, studies only about $1$ topic among $K$ topics.
    \item In the second scenario, we consider three cases where we have
        $\{(n^{(i)},n_{2}^{(i)},n_{3}^{(i)},K^{(i)},|S^{*}|^{(i)})\}_{i=1}^{3}=
    ...
    \item In the third scenario, we basically study the same $3$ cases with those from the second scenario.
        (i.e., $\{(n^{(i)},n_{2}^{(i)},n_{3}^{(i)},K^{(i)},|S^{*}|^{(i)})
        \}_{i=1}^{3}=\{(120,0,10,3,18), (210,50,0,3,18),(210,10,10,3,18)\}$)
        The only difference between these two scenarios comes from the random setting of $\alpha^*$. (i.e.,$\alpha^* \sim \text{Unif}[-4,-3]$)
\end{enumerate}

All $9$ networks that are elaborated in scenarios $1$, $2$ and $3$ are visualized in Fig\ref{fig:figure1}.
Note that ad-hoc links, which connect clusters of papers with $1$ topic, are colored in green.
...
Following the scree-plot approach in Ji and Jin \cite{ji2016coauthorship}, we plot the largest $20$ eigenvalues of the adjacency matrix $X$, and find an ``elbow'' point where the eigenvalues seem to level off.
...
(We will denote this number as $\hat{K}^{\text{Scree}}$.)
...
We need to go through several iterations of this recording procedure to find a proper range of grid, in which we can get $\text{rank}(\widehat{L}^{\gamma,\delta})=\hat{K}^{\text{Scree}}$
...
\[
    ...
    \big\{(\gamma,\delta): \text{rank}(\widehat{L}^{\gamma,\delta})=\hat{K}^{\text{Scree}},
    \quad \mbox{mode} |\widehat{S}^{\gamma,\delta}| \quad
    \text{s.t.} \quad 10^{-4}|X| \leq |\widehat{S}^{\gamma,\delta}| \leq 0.05|X| \big\}
\]
...
\caption{ Scree plots for $9$ synthetic Networks.
    $\hat{K}^{\text{Scree}}$ recovers the number of topics in network correctly for all $9$ cases.}
\label{fig:figure2}
\end{figure}

\begin{eqnarray*}
M_1 &=& \mathbbm{1}\big\{\text{rank}(\widehat{L}) = \text{rank}(L^{*})\big\}, \\
...
We present the evaluation results of $9$ cases via these $3$ criteria in Table \ref{tab:table1}.
...
\begin{enumerate}
    ...
    First and foremost, choosing a good pair of tuning parameters is of essential issue when it comes to making a good statistical inference on data.
    As presented in Table\ref{tab:table1}, we observe that the traditional model selection methods, both BIC and AIC, perform poorly on choosing true models, whereas our heuristic approach chooses good models reasonably well for the 9 artificial cases.
    In the heuristic approach, scree-plot plays an important role when it comes to recovering the number of topics for all $9$ cases. (See Fig\ref{fig:figure2})
    On the other hand, both BIC and AIC choose the most parsimonious model with the smallest $K(=\text{rank}(\widehat{L}))$ and the smallest cardinality$(=|\widehat{S}|)$ of ad-hoc links among the estimated ($\widehat{L}$,$\widehat{S}$)s' over the given grid.

    \item \textbf{Node Membership.}
    After fitting the model with a proper pair of tuning parameters, $(\gamma,\delta)$, we need to determine whether the $i$th paper belongs to $k$th topic or not.
    We apply a simple $k$-means clustering algorithm on the $\widehat{L}$ matrix’s $K(=\text{rank}(\widehat{L}))$ eigenvectors.
    (We will denote column-wise concatenation of these $K$ eigenvectors as $\widehat{E}_{K} \in \mathbb{R}^{n\times K}$.)
    For $3$ cases in Scenario $1$, where each paper in the network can only have $1$ topic, we confirm that $k$-means algorithm performs well on classifying papers in the network.
    However, in Scenario $2$ and $3$ where we allow the papers in the network can have more than $1$ topic, naive implementation of $k$-means algorithm entails a problem, since it is not able to afford the overlapped membership of nodes.
    ...
    See Fig\ref{fig:figure3} for the references on how this procedure performs on $9$ synthetic cases.
...
\begin{tabular}{c|ccc|ccc|ccc}
            & \multicolumn{9}{c}{\textbf{Scenario 3}}                                              \\
            \cline{2-10}
            & \multicolumn{3}{c|}{Case 7} & \multicolumn{3}{c|}{Case 8} & \multicolumn{3}{c}{Case 9} \\
            \cline{2-10}
            & Heuristic   & AIC   & BIC  & Heuristic   & AIC   & BIC  & Heuristic   & AIC   & BIC  \\
            \hline
$M_{1}$  &    1(3)   &   1(3)    &  1(3)    &     1(3)        &    1(3)   &  1(3)    &   1(3)          &  1(3)      &  1(3)     \\
$M_{2}$  &     1       &   0  &    0  &    $\frac{15}{18}$         &   0  &  0  &   1   &  1    &   1     \\
$M_{3}$  &  $\frac{1}{7122}$  &  0  &  0   &   $\frac{3}{21927}$ &     0  &  0     &  $\frac{4}{21927}$ & $\frac{4}{21927}$ & $\frac{4}{21927}$ \\
%Grid Region &             &       &      &             &       &      &             &       &      \\
\end{tabular}
...
\caption{ Plots of the projected points on the first (X-axis) and second (Y-axis) principal component of the data matrix $\widehat{E}_{K}$. Different colors represent different clusters of papers that $k$-means algorithm assigns.}
\label{fig:figure3}
\end{figure}
...
\caption{ Scree plots for 543-Citation network, and Sub-network.}
\label{fig:figure4}
\end{figure}
...
A pair of parameters, $(\gamma^{\text{Heu}},\delta^{\text{Heu}}) = (0.000912,0.0097)$, gives us $\widehat{L}$ with rank $3$, and $\widehat{S}$ with $|\widehat{S}|=197$.
