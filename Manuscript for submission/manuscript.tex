...
Our experience shows that both BIC and AIC choose the most parsimonious model with the smallest $K(=\text{rank}(\widehat{L}))$ and the smallest cardinality$(=|\text{supp}(\widehat{S})|/2)$ of ad-hoc dependency among all the estimated ($\widehat{L}$,$\widehat{S}$)s'.
