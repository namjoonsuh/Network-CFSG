...
\label{sec:Syn-dat}
After a brief introduction on several synthetic scenarios that we want to explore, we will discuss how to set ground truth parameters for generating synthetic networks from pre-specified scenarios.
Subsequently, three model selection criteria and three evaluation metrics for the selected model will be introduced.
Lastly, several interesting observations from the experiments will be presented.
...
In second scenario, we study a slightly more complicated situation than the first one where each paper in the network can possibly have more than one topic.
...
More concretely, in first scenario, when there is a network of $n$ papers with $K$ topics, there are $K$ clusters of papers, where each of which deals with $1$ specific topic out of $K$ topics.
...
Based upon the settings mentioned in the first scenario, additionally in second scenario, we suppose that there are $n_{l}$ papers, which share $l(\leq K)$ common topics in the network.
For instance, if we have a network of $210$ papers with $3$ topics, and
$n_2=50$, then $n_2=50$ means that there are $50$ papers out of $210$ papers, which have $2$ shared topics out of $3$ topics.
...
\begin{enumerate}
    ...
    \item In the second scenario, we consider three cases where we have
        ...
        Note that $3$ clusters from these $190$ papers are connected through $18$ random ad-hoc links.
        % For all $3$ cases, networks considered in this scenario have $3$ topics, and $3$ clusters of papers are connected via $18$ random ad-hoc links.
        % In the first case, we generate a network with $120$ papers in total, which is comprised of $110$ papers with $1$ topic, and $10$ papers with $3$ mixed topics.
        % In the second case, we have a network with $210$ papers in total.
        % Among them, $50$ papers randomly share $2$ topics out of $3$ topics.
        % And remaining $160$ papers talk about only $1$ topic.
        % Lastly, in the third case, the network to be considered has $210$ papers in total.
        % Among them, $10$ papers randomly share $2$ topics out of $3$, other $10$ papers have $3$ mixed topics, whereas remaining $190$ papers only discuss $1$ topic.
    \item adsfasdf
\end{enumerate}
...
For more detailed descriptions on parametric settings for generating networks in Fig\ref{fig:figure1}, readers can refer the R code provided in $\dots$.
%In next subsection, we provide detailed ground truth parameters settings for generating the networks in Fig\ref{fig:figure1}.
...
Following the approach in Ji and Jin \cite{ji2016coauthorship}, we plot the largest $20$ eigenvalues of adjacency matrix $X$, which can tell us the number of communities embedded in the network
(See Fig\ref{fig:figure2} to check for the cases of $6$ aforementioned synthetic networks).
Then, we record the rank of $\widehat{L}$ and number of non-zero elements in $\widehat{S}$ for each tuning parameter pair on a given grid, and choose a proper pair that gives us interesting interpretations of data.
...
Our experience shows that both BIC and AIC choose the most parsimonious model with the smallest $K(=\text{rank}(\widehat{L}))$ and the smallest cardinality$(=|\widehat{S}|)$ of ad-hoc links among all the estimated ($\widehat{L}$,$\widehat{S}$)s' over the given grid.
