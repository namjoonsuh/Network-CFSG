...
In section \ref{sec:estimate}, we will discuss about the assumptions in our model and penalization on likelihood function constructed in section \ref{sec:model-form}.
...
We finally conclude this work in section \ref{sec:Con} with several open questions remain to be solved and some directions for future research.
...
We use a simple gradient descent to solve this step, setting the step size equal to $0.1$ and stopping criteria as $\max(|x_{\alpha,m}^{(t+1)}-x_{\alpha,m}^{(t)}|,\|x_{M,m}^{(t+1)}-x_{M,m}^{(t)}\|_{\infty}) \leq 10^{-9}$.
...
We terminate the algorithm  when in $m$th iteration, $\|x^{m}_M - x^{m}_L - x^{m}_S\|_F \leq \delta$, with $\delta=10^{-7}$.

\section{ Numerical analysis and Applications}
\label{sec:Num-anal}
In this section, we have conducted a small empirical study of measuring the performance of our proposed method with artificially specified graphical structures,\ref{sec:Syn-dat}.
Also we perform a real data analysis with citation network for statisticians,\ref{sec:citation_network}.
...
\begin{enumerate}
    ...
    \item Each element of diagonal matrix $D^{*}$ are generated from Unif$[7,8]$.
    \item For the sparse component $S^*$, first draw $nnz$ distinct random pairs of indices that are uniformly distributed over the indices of upper triangular part (off diagonal) of matrix $S^*$. For each drawn pair of indices $(i,j)$, draw random number from Unif$[7,8]$ (i.e., $S^*_{ij}\sim\text{Unif}[7,8]$). $nnz$ denotes the number of non-zero component of the upper-triangular part of $S^{*}$.
\end{enumerate}
...
We have to admit the fact that choosing a good pair of tuning parameters is an important but challenging issue in our problem.
Here we present a purely heuristic approach.
...
Then, we record the rank of $\widehat{L}$ and number of non-zero elements in $\widehat{S}$ for each tuning parameter pair on grid, and choose a proper pair that give us interesting interpretations of data.

One might wonder how traditional model selection methods, such as the Bayes Information Criterion (BIC;\cite{schwarz1978estimating}) or the AIC, work where we define BIC and AIC as follows:
\[
...
For clarification on this argument, we evaluate the models selected via our heuristic approach, BIC, and AIC by using following three evaluation metrics.
...
Note that both AIC and BIC choose the model with no ad-hoc structure (i.e., $\widehat{S}=0$),
and $\widehat{L}$ with the lowest rank among all estimators via a pair of tuning parameter over the grid of search region.}
\label{tab:table1}
\end{table}
...
For the illustration of performance of our fitted model after applying $k$-means algorithm on three synthetic scenarios, in Fig \ref{fig:figure1}, we plot the coordinates of each rows of first two eigen-vectors on $2D$ plane.
...
Among  $3248$ papers in total, we restrict our attention on papers which have greater than or equal to $7$ citations from the collected papers in original dataset.
We have $543$ papers in our hands.
The elbow points of the scree plot may be at the $3$rd, $7$th, or $9$th largest eigenvalue, suggesting that there might be from $2$ to $8$ topics in network (See $Fig \ref{fig:figure3}, left$).
...
A pair of parameters, $(\gamma,\delta) = (0.000912,0.0097)$, gives us $\widehat{L}$ with rank $3$, and $\widehat{S}$ with $|supp(\widehat{S})|/2=197$.
...
For further investigation on this group, after obtaining the sub-network which is comprised only with papers in third group, we performed same analysis once again under the assumption that $K=4$ (See $Fig \ref{fig:figure3},right$).
...
Furthermore, it is worth to note that our result is consistent with that from Ji and Jin \cite{ji2016coauthorship} in a sense that they also recover ``Multiple Testing'', ``Variable selection'' and ``non-parametric Bayesian statistics''.
...
\end{tabular}
\caption{15 pairs of papers that have the largest estimated $\widehat{S}_{ij}$.
...
We propose a new combined latent factor and logistic regression model.
...
Nonetheless, there are still several open questions remaining to be answered.
